...
def get_video_id_from_search(query: str):
    ...
    if API_KEY == "YOUR_API_KEY": return None, "Server Error: YouTube API key not configured."
    try:
...
def create_or_join_room():
    ...
    user_ref = db.collection('users').document(user_info['id'])
    user_ref.set({'rooms': firestore.ArrayUnion([room_code])}, merge=True)
...
def rejoin_room(room_code):
    ...
    if user_doc.exists:
        ...
        if user_rooms and room_code not in user_rooms:
...
def get_online_members(room_code):
    """Helper to get a list of online members in a room."""
    online = []
    for sid in sid_to_user:
        if sid_to_user[sid]['room'] == room_code:
            online.append(sid_to_user[sid]['username'])
    return online
        ...
        if room_doc.exists:
            ...
            if room_data.get('current_video'):
                ...
                if video_state['state'] == 'playing':
                    elapsed_time = time.time() - video_state['last_update']
                    video_state['time'] += elapsed_time
...
def handle_disconnect():
    ...
    if request.sid in sid_to_user:
        user_info = sid_to_user[request.sid]
